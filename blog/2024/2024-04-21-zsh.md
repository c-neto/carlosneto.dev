...
When the acttivity is remember history commands, the shells implemented widgets called by `CTRL` + `R` shortcut. In Zsh is implemented _reverse-i-search_ which shows the commands limited by one result. You need to press the shortcut again to see next results. This behavior can be a problem if you need to recall commands executed multiple times with differents arguments.
